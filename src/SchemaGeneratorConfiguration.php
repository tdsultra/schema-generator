        ...
        $treeBuilder
            ...
            ->children()
                ...
                ->arrayNode('id')
                    ...
                    ->children()
                        ->scalarNode('name')->defaultValue('id')->info('The property name of the ID')->example('id')->end()
                ...
                ->scalarNode('inflector')
                    ->defaultValue(InflectorProxy::SYMFONY_INFLECTOR)
                    ->info('The inflector to use during class generation')
                    ->example('doctrine (if you want to use Doctrine inflector) symfony (if you want to use Symfony inflector)')->end()
                        ...
                        ->children()
                            ->arrayNode('pk')
                                ...
                                ->children()
                                    ...
                                    ->booleanNode('generate')->defaultTrue()->info('Override: Automatically add an id field to entities')->end()
                                    ->enumNode('generationStrategy')->defaultValue('auto')->values(['auto', 'none', 'uuid', 'mongoid'])->info('Override: The ID generation strategy to use ("none" to not let the database generate IDs).')->end()
                                    ->booleanNode('writable')->defaultFalse()->info('Override: Is the ID writable? Only applicable if "generationStrategy" is "uuid".')->end()
                                    ->enumNode('onClass')->defaultValue('child')->values(['child', 'parent'])->info('Override: Set to "child" to generate the id on the child class, and "parent" to use the parent class instead.')->end()
                                ->end()
                            ->end()
                            ...
                            ->scalarNode('shortName')->defaultFalse()->info('An optional short name for this class')->end() // @COREMOD
                            ->arrayNode('namespaces')
                                ->addDefaultsIfNotSet()
                            ...
                            ->arrayNode('properties')
                                ...
                                ->arrayPrototype()
                                    ...
                                    ->children()
                                        ...
                                        ->booleanNode('readableLink')->defaultTrue()->info('Is the property eager-loaded via ApiPlatform?')->end()  // @COREMOD
                                        ->booleanNode('writable')->defaultTrue()->info('Is the property writable?')->end()
                                        ->booleanNode('writableLink')->defaultTrue()->info('Is the property writable via ApiPlatform?')->end()  // @COREMOD
                                        ->booleanNode('formatName')->defaultTrue()->info('Should this name be formatted?')->end()  // @COREMOD
                ...
                ->arrayNode('generatorTemplates')
                    ...
                    ->scalarPrototype()->end()
                ->end()
                ->scalarNode('classTemplates')->defaultFalse()->info('Directory for base Nette class templates')->end()
            ->end();
